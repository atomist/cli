...
type DockerRegistryProviderTypes = Record<string, {
    label: string;
...
const DockerRegistryProviders: DockerRegistryProviderTypes = {
    dockerhub: {
        label: "Docker Hub",
        create: createDockerHub,
    },
    jfrog: {
        label: "Docker Hub",
        create: createJFrog,
...
export async function config(opts: ConfigureOptions): Promise<number> {
    ...
    print.log("Select an Docker registry provider type to configure:");
    const questions: inquirer.Question[] = [
        {
            type: "list",
            name: "provider",
            message: "Docker provider",
            choices: _.map(DockerRegistryProviders, (v, k) => ({ name: v.label, value: k })),
        },
    ];

    const answers = await inquirer.prompt(questions);
    try {
        const result = await DockerRegistryProviders[answers.provider].create(workspaceId, apiKey, cfg);
        const newCfg = {
            ...userCfg,
            ...(result.configuration || {}),
        };
        await writeUserConfig(newCfg);
        print.log(`Successfully configured Docker registry provider ${chalk.cyan(DockerRegistryProviders[answers.provider].label)}`);
        return result.code;
    } catch (e) {
        print.error(`Failed to configure Docker registry provider: ${e.message}`);
        return 1;
    }
}
