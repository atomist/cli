 ...
 */
export interface RepositoryStartOptions {
    cloneUrl: string;
    index: string;
    sha: string;
    local: boolean;
...
export async function repositoryStart(opts: { cloneUrl: string } & Partial<RepositoryStartOptions>): Promise<number> {
    ...
    try {
        packageJsonCreated = await copyFiles(optsToUse, cwd, seed);
    } catch (e) {
    ...
    try {
        if (!(await fs.pathExists(path.join(cwd, "package-lock.json"))) && !packageJsonCreated) {
            await mergePackageJson(cwd, seed);
        }
    } catch (e) {
        print.error(`Failed to merge 'package.json': ${e.message}`);
        return 20;
    }
    ...
    // Finally called start on the constructed SDM/client
    return start({
        ...optsToUse,
        cwd,
    });
...
async function copyFiles(optsToUse: RepositoryStartOptions, cwd: string, seed: string): Promise<boolean> {
    const gitUrl = gitUrlParse(optsToUse.cloneUrl);
    ...
    return packageJsonCreated;
}

async function mergePackageJson(cwd: string, seed: string): Promise<void> {
