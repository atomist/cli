...
type ScmProviderTypes = Record<string, {
    label: string;
...
const ScmProviders: ScmProviderTypes = {
    github_com: {
        label: "GitHub.com",
    ...
    const questions: inquirer.Question[] = [
        {
            ...
            choices: _.map(ScmProviders, (v, k) => ({ name: v.label, value: k })),
        },
    ];

    const answers = await inquirer.prompt(questions);
    try {
        const result = await ScmProviders[answers.provider].create(workspaceId, apiKey, cfg);
        const newCfg = {
            ...userCfg,
            ...(result.configuration || {}),
        };
        await writeUserConfig(newCfg);
        print.log(`Successfully configured SCM provider ${chalk.cyan(ScmProviders[answers.provider].label)}`);
        return result.code;
    } catch (e) {
